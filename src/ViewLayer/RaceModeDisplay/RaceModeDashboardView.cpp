    ...
    , faultsTimer_(new QTimer())
{
    ...
    connect(backgroundAnimation_, &QAbstractAnimation::finished, this, &RaceModeDashboardView::reverseBackgroundAnimation);
    ui_.show();
}
...
void RaceModeDashboardView::updateFaultLabel(QLabel& dashboardLabel, FaultLabel faultLabel)
{
    if (faultLabel.priority() >= 0)
    {
        ...
        if (backgroundAnimation_->currentTime() == 0 && dashboardLabel.text() != faultLabel.text())
        {
